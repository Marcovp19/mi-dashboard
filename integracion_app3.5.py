            ...
            if df_promoters_summary.empty:
                ...
                if filtered_promoters.empty:
                    ...
                    if df_match.empty:
                        ...
                        # --- Nº de Clientes Activos (VENCI ≥ hoy) -------------------------------
                        hoy = datetime.now().date()
                        df_pagos_prom_raw = df_pagos_raw[df_pagos_raw["N"] == promotor_sel]
                        clientes_activos = (df_pagos_prom_raw["VENCI"].dt.date >= hoy).sum()
                        st.markdown(f"**Nº de Clientes Activos:** {clientes_activos}")

                        from datetime import datetime      # ← ya existe al principio; si no, añádelo una sola vez

                        # --- Nº de Clientes Vencidos --------------------------------------------
                        #  • Crédito ya VENCIDO  →  VENCI < hoy
                        #  • MULTAS (SV) > 0
                        hoy = datetime.now().date()

                        mask_vencido = (
                            (df_pagos_prom_raw["VENCI"].dt.date < hoy) &    # fecha de vencimiento pasada
                            (df_pagos_prom_raw["SV"] > 0)                   # con multas > 0
                        )
                        clientes_vencidos = int(mask_vencido.sum())
                        st.markdown(f"**Número de Clientes Vencidos:** {clientes_vencidos}")

                        # --- Saldo Vencido Total (suma de SV) -----------------------------------
                        saldo_vencido_total = df_pagos_prom_raw["SV"].sum()
                        st.markdown(f"**Saldo Vencido Total:** {format_money(saldo_vencido_total)}")
                        from datetime import datetime   # (ya está al inicio del archivo; si no, añádelo una sola vez)

                        # --- Nº de Clientes Atrasados -------------------------------------------
                        #  • Crédito NO vencido  →  VENCI >= hoy
                        #  • MULTAS  >  PS
                        hoy = datetime.now().date()
                        mask_atraso = (
                            (df_pagos_prom_raw["VENCI"].dt.date >= hoy) &       # no vencido
                            (df_pagos_prom_raw["SV"] > df_pagos_prom_raw["PS"]) # multas mayores que PS
                        )

                        clientes_atrasados = int(mask_atraso.sum())
                        st.markdown(f"**Nº de Clientes Atrasados:** {clientes_atrasados}")
