...
def build_ranking(df_control, df_metas_summary, df_cobranza):
    """
    ...
    """
    df_metas_no_ultima = df_metas_summary.copy()
    ...
    df_cob_group = df_cobranza.groupby("Nombre Promotor")["Depósito"].sum().reset_index(name="Cobranza_total")
    ...
    df_ranking["N_prom_numeric"] = pd.to_numeric(
        df_ranking["N"].astype(str).str.extract(r"(\d+)")[0],
        errors="coerce"
...
def main():
    ...
    if vas_file and cob_file:
        try:
            ...
            df_desc_agg = load_data_descuentos(por_capturar_file)  # puede ser vacío si no se sube
        ...
        # --------------------------------------------------------------------------------
        # --- 2. Pestaña: Ranking de Promotores (excluyendo última meta) ---
        with tabs[2]:
            ...
            df_ranking = build_ranking(df_control, df_metas_summary, df_cobranza)
        ...
        with tabs[3]:
            ...
            st.markdown("""
                Se listan promotores cuya diferencia en el día promedio sea >= 0.65:
                - Rojo si >=1.1
                - Amarillo si entre 0.65 y 1.1
            """)

            change_rows = []
            for _, row in df_promoters_summary.iterrows():
                promoter_code = row["N"]
                ...
                # Tomamos las últimas 7 semanas, si existen.
                if n >= 7:
                ...
                # Se incluyen desde 0.65
                if diff >= 0.65:
                    change_rows.append({
                        "N": promoter_code,
                        "Nombre": promoter_name,
                        "Inicio Promedio": round(first_avg, 2),
                        "Final Promedio": round(last_avg, 2),
            ...
            if change_rows:
                ...
                st.write("No se detectaron promotores con diferencia >= 0.65 en el patrón de pago.")
        ...
        with tabs[5]:
            ...
            if not df_promoters_summary.empty:
                ...
                if search_term:
                    # Filtrar promotores cuyo nombre contenga el término (ignora mayúsculas/minúsculas)
                ...
                if len(promoter_names) == 0:
                    ...
                    if df_match.empty:
                        ...
                        st.markdown(
                            ...
                            f"**Estado(s):** {estado_str}  \n"
                            f"**Municipio(s):** {municipio_str}  \n"
                            f"**Cobranza Meta (Total):** {format_money(total_cobranza_meta)}  \n"
                            f"**Cobranza Realizada (Total):** {format_money(total_cobranza_real)}  \n"
                            f"**Diferencia (Total):** {format_money(diferencia_cobranza)}"
                        )
                        
                        # --- Sección de Resumen Semanal y Detalle Diario ---
                        # Agrupar la cobranza por semana
                        df_cob_summary = df_cob_prom.groupby("Semana")["Depósito"].sum().reset_index()
                        
                        # Determinar el rango de semanas usando tanto las metas como la cobranza
                        ...
                        elif not df_meta_prom.empty:
                            start_week = df_meta_prom["Semana"].min()
                            end_week = df_meta_prom["Semana"].max()
                        else:
                            start_week = pd.to_datetime("today").to_period("W-FRI")
                            end_week = start_week
                        
                        # Crear un DataFrame con todas las semanas del rango
                        ...
                        # Unir la información de metas y cobranza
                        ...
                        # Permitir al usuario seleccionar el número de semana para ver el detalle diario
                        week_num = st.number_input("Ingresa el número de semana para ver detalles diarios", min_value=1, step=1, value=1, key="week_num_detalle")
                        if week_num <= len(df_weeks):
                            sel_week = df_weeks.loc[df_weeks["Número de Semana"] == week_num, "Semana"].iloc[0]
        ...
        with tabs[7]:
            ...
            if df_col_merge.empty:
                st.write("No se han encontrado datos de colocaciones. Por favor, sube el archivo 'Colocaciones_al_7_mar.xlsx'.")
            else:
                ...
                if len(promoter_names) == 0:
                    ...
                    if df_match.empty:
                        ...
                        if df_sel.empty:
                            ...
                            # Renombramos la columna para el merge
