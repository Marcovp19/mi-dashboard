...
def main():
    ...
    if vas_file and cob_file:
        try:
            ...
            df_cobranza = map_promoter_names_to_codes(df_cobranza, "Nombre Promotor", df_control)
            # La función map_promoter_names_to_codes ya limpia sus propias columnas temporales.

            # --- Carga de datos de colocaciones (agregado y detallado) ---
            df_col_agg, df_col_detail_return = load_data_colocaciones(col_file) 
            ...
            if df_col_detail_return is not None and not df_col_detail_return.empty:
                if "Nombre promotor" in df_col_detail_return.columns:
                    df_colocaciones_info_completa = map_promoter_names_to_codes(
                        df_col_detail_return, 
                        "Nombre promotor", 
                        df_control
                    )
                else:
                    # Si falta la columna 'Nombre promotor', no se puede mapear.
                    # load_data_colocaciones ya debería haber manejado esto o devuelto un df vacío/con error.
                    # Aquí solo nos aseguramos que df_colocaciones_info_completa tenga la columna 'N'.
                    st.warning("La columna 'Nombre promotor' no se encontró en los datos de colocaciones detallados. No se pudo realizar el mapeo a códigos 'N'.")
                    df_colocaciones_info_completa = df_col_detail_return.copy() # Usar la copia de los detalles
                    if "N" not in df_colocaciones_info_completa.columns:
                         df_colocaciones_info_completa["N"] = pd.NA
            else:
                # Si df_col_detail_return está vacío o es None
                df_colocaciones_info_completa = pd.DataFrame(columns=df_col_detail_return.columns if df_col_detail_return is not None else None)
                df_colocaciones_info_completa["N"] = pd.NA # Asegurar columna N
            ...
            df_col_merge = merge_colocaciones(df_col_agg, df_control)
             ...
             df_pagos_raw = map_promoter_names_to_codes(df_pagos_raw, "PROMOTOR", df_control)
             # map_promoter_names_to_codes ya limpia sus propias columnas temporales.


             # Agrupamos df_pagos por el "N" ya mapeado
                df_pagos = (
                    df_pagos_raw
                    .dropna(subset=["N"]) # Usar la columna "N" mapeada
