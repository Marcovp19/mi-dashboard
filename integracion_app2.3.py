...
def main():
    ...
    if vas_file and cob_file:
        ...
        with tabs[0]:
            ...
            if df_metas_summary.empty or df_cobranza.empty:
                ...
                if len(all_weeks) == 0:
                    ...
                    # Gráfica de barras (Metas vs Cobranza)
        ...
        with tabs[3]:
            ...
            st.markdown("""
            Se compara el día promedio de pago, calculado como un promedio ponderado del Día_num 
            (sábado=1, domingo=2, lunes=3, etc.) por los depósitos de cada semana.
            Luego se ve la diferencia entre las últimas semanas y las primeras.
            - Rojo si diferencia >= 1.1
            - Naranja si diferencia >= 0.65 y < 1.1
            - Sin color si < 0.65
            """)

            # --------------------------------------------------------------
            # 1) Construimos df_change
            # --------------------------------------------------------------
            code_to_name = dict(zip(df_control["N"], df_control["Nombre"]))
            all_prom_changes = []

            for code, name in code_to_name.items():
                # Filtramos la cobranza del promotor
                df_prom = df_cobranza[df_cobranza["Nombre Promotor"] == name.upper()].copy()
                if df_prom.empty:
                    # Si ni siquiera hay depósitos para este promotor, se ignora
                    continue

                # Calculamos weighted_product = Día_num * Depósito
                ...
                # Necesitamos al menos 2 semanas para ver un "cambio"
                if n < 2:
                    continue

                # Tomamos las últimas 6 semanas si hay suficientes, para comparar la "mitad inicial" vs. "mitad final"
                if n >= 6:
            ...
            # --------------------------------------------------------------
            # 2) Si df_change está vacío, mostramos aviso. Si no, aplicamos estilo.
            # --------------------------------------------------------------
            if df_change.empty:
                st.write("No hay datos suficientes para mostrar cambios de patrón de pago.")
            else:
                # Definimos una función para colorear solo el texto
                def style_day_diff(val):
                    if pd.isna(val):
                        return ""
                    # Rojo si val >= 1.1
                    if val >= 1.1:
                ...
                # Aplicamos este estilo SOLO a la columna "Diferencia"
                styled_change = df_change.style.applymap(style_day_diff, subset=["Diferencia"])
