...
def main():
    ...
    if vas_file and cob_file:
        try:
            ...
            # Mapear nombres a código "N" usando df_control
            name_to_code = dict(zip(df_control["Nombre_upper"], df_control["N"]))
            df_pagos_raw["N"] = df_pagos_raw["PROMOTOR"].map(name_to_code)

            # Agrupar para quedar con saldos totales por código
            df_pagos = (
                df_pagos_raw
                .dropna(subset=["N"])               # descartamos si no pudo mapear
        ...
        with tabs[0]:
            ...
            if df_metas_summary.empty or df_cobranza.empty:
                ...
                total_cartera_hist = df_pagos["SALDO"].sum()    # <-- calculamos la cartera total
